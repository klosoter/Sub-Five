...
@app.route("/create")
def create_room():
    print("CREATE ROOM: session cleared")
...
@app.route("/join/<code>")
def join_room(code):
    print("JOIN ROUTE:", code, "session =", dict(session))
...
def post_join_room():
    print("POST JOIN:", request.form)
    print("SESSION SET:", session.get("room"), session.get("player_name"))
...
def end_room():
    ...
    print("END ROOM:", code, "by player", player_name)
...
def toggle_ready(code):
    print("TOGGLE READY:", code, "session =", dict(session))
    print("Room players:", rooms[code]["players"], "Ready:", rooms[code]["ready"])
...
@app.route("/state")
def state():
    print("STATE CHECK: session =", dict(session))
    code = session.get("room")
    player_name = session.get("player_name")
    room = rooms.get(code)
